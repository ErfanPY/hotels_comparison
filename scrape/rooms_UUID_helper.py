        ...
        for room_counter, room in enumerate(all_rooms):
            ...
            if not room_type == " ":
                if not room_type == room['romType']:
                    try:
                        room_type_update_query = "UPDATE tblRooms SET romType='{}' WHERE romID = {}".format(
                            room_type, room['romID'])
...
with get_db_connection() as conn:
    ...
    rooms_all_UUID = custom(query_string=rooms_all_UUID_query, conn=conn)

used_room_UUID = set([i['romUUID'] for i in rooms_all_UUID])
# used_room_UUID = set()
room_index_ID = {}
...
def print_rooms(room_index_ID, htlUUID, site_rooms):
    ...
    with open('rooms.txt', 'w') as f:
        ...
        site_rooms = dict(
            sorted(site_rooms.items(), key=lambda item: len(item[1])))
...
for htlUUID, rooms in hotel_rooms.items():
    ...
    # Try to find UUID automatically

    for room in list(site_rooms.values())[1]:
        for i, check_room in enumerate(list(site_rooms.values())[0]):
            price_check = abs(room['avlBasePrice'] -
                              check_room['avlBasePrice']) < 500000
